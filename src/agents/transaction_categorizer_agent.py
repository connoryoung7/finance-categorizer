...
class CategorizeTransactionAgent:
    """
    This is an agent that is responsible for retrieving transaction data
    and the categories that the transaction could be categorized as
    and assigning a category for the transaction.
    """
    ...
    def categorize_transaction(self, transaction: Transaction) -> None:
        if transaction.amount >= 0:
            print(
                f"Skipping categorization for transaction "
                f"{transaction.id} as it is not an expense."
            )
            return
        ...
        if category_id:
            print(
                ...
                f"by payee history to category {category_id}"
            )
            self.transaction_service.categorize_transaction(
            ...
            print(
                ...
                f"with confidence {result.confidence_level}"
            )
    ...
    def categorize_from_web_search_internet(
        ...
        search_results = self.__search_payee_name_on_internet(
        ...
        response = self.mistral_client.chat.parse(
            ...
            messages=[
                SystemMessage(
                    content="""
                    You are a data labeller for a credit card company.
                    You will be given all the possible categories that a
                    transaction can be in the form of JSON. You will also
                    be given the search results for the payee name on the
                    internet. By looking at the different search results,
                    you want to determine what the category of the
                    transaction is. In addition to your categorization,
                    you will also provide a confidence level which will
                    denote how likely this categorization is. For context,
                    if you can guarantee that the payee name matches the
                    category, you should provide a confidence level of
                    0.95 or higher. If there is some uncertainty, you
                    should provide a confidence level between 0.90 and
                    0.95. If you are unsure, you should provide a
                    confidence level below 0.90.
                    """
                ),
                UserMessage(
                    content=(
